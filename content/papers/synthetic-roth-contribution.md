...
**Passman, Wong, & Dickson (2025)** — *A "BETR" Approach to Roth Conversions*, Vanguard (July 2025) — compute a break-even future tax rate (BETR) for a Roth conversion paid from outside funds. Their flagship case study walks through Jill (35% current bracket, 24% future, \$100,000 conversion, 20-year horizon, \$35,000 conversion tax): the conversion-tax dollar is grown in a hypothetical taxable account (\$35K → \$70K) and subtracted from the Roth side as a "forgone after-tax value": \$300,000 (full Roth growth) − \$70,000 = \$230,000. The no-conversion case is \$300,000 traditional IRA growth at the future rate, with the \$35,000 that would have stayed in the bank omitted entirely. Setting the two equal yields BETR = 23.3% — Jill should convert because her 24% future rate exceeds it. The rate is mathematically correct because both sides symmetrically omit the conversion-tax dollar, but the math never places the \$35,000 inside the Roth wrapper — despite Vanguard's narrative that the conversion *"allows more dollars to be placed in a tax-advantaged account."* The SRC framework resolves the tension by treating the migration as explicit and first-class: the \$35,000 lives inside the Roth with present value $K$ at the portfolio rate (§4), additive to the rate-spread component rather than embedded in a BETR adjustment.

The two parameterizations are mathematically related. Vanguard's growth-multiple difference between tax-efficient and tax-inefficient outside accounts maps to the SRC's per-year drag fraction $d$ via $d = 1 - (M')^{1/n}/M^{1/n}$ (§6), where $M$ and $M'$ are our notation for the no-drag and drag-affected horizon-cumulative growth multiples respectively (Vanguard does not name these parameters). The per-year framing is more actionable in practice: advisors can estimate $d$ directly from a client's outside-account tax profile — asset location, turnover, distribution yield, state of residence — and explain it as "the share of annual return lost to taxation," a quantity observable on prior tax returns. The horizon-cumulative growth multiple, by contrast, requires projecting a 20-year wealth differential against a hypothetical no-drag counterfactual — an opaque input the advisor must project and the client cannot verify.
