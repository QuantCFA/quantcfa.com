...
**Reichenstein & Meyer (2017)** — *Valuing Roth Conversion and Recharacterization Options*, Journal of Financial Planning 30(11): 48–56 — express the value advantage of paying conversion tax from outside funds as $tV(1+r)^n - tV(1+R)^n$, where $tV$ is the conversion tax ($= K$), $r$ is the Roth pre-tax return, $R$ is the after-tax return on a taxable account, and $n$ is the horizon. They observe directly: *"Strategy 3 has tV more dollars growing at the pre-tax return of r in the Roth IRA."*

That observation IS the SRC mechanism. The first term $tV(1+r)^n$ is the migrated tax dollars compounding tax-free in the Roth — the dominant component of outside-funded conversion value, with present value $K$ at the portfolio rate (§4). But the framework immediately nets it against the bank's forgone growth $tV(1+R)^n$, leaving only the much smaller tax-drag residual; at $R = r$ the expression collapses to zero and Outside registers no benefit. This is the McQuarrie pattern in different notation: the SRC appears in the author's math, then is netted away against an opportunity cost (McQuarrie discounts RMD-tax savings against the conversion tax; Reichenstein & Meyer net Roth growth against taxable-wrapper growth). Both authors report the residual — drag-alpha for Reichenstein & Meyer, NPV near zero for McQuarrie — without recognizing the wrapper migration as a separable Roth-equivalent contribution.

Reichenstein & Meyer's contribution to the drag-alpha component of conversion value is real and prior; the SRC framework treats their $tV(1+r)^n - tV(1+R)^n$ as a special case of the §6 tax-drag-avoidance alpha and builds the dominant wrapper-migration component on top.
