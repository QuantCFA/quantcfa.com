...
**Comparison to FA.** For the Outside FA scenario, the lifetime nominal payout = \$17,966 × 1.88786 = \$33,917.27. The RMD multiplier (3.9647) exceeds the FA multiplier (1.88786) by ~2.1× because RMD's distribution horizon is longer (30 years vs 20 for FA): the SRC's $K$ dollars compound more years inside the Roth before being fully distributed, producing larger nominal cash flows over the longer horizon. In PV terms at the portfolio rate, both methods produce the same SRC value of zero by §4's identity — the difference is purely in nominal lifetime cash flow, not in SRC economic value.
