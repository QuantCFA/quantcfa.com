...
**Passman, Wong, & Dickson (2025)** — *A "BETR" Approach to Roth Conversions*, Vanguard (July 2025) — come closest to recognizing the SRC when they observe that *"paying conversion taxes from a taxable account lets investors move some of their savings (the amount of the conversion taxes) from a taxable account to a Roth account."* Yet their BETR construction submerges this insight through a symmetric omission of the conversion tax $K$ from both sides of the comparison. The math is invariant — one could equally retain $K$ on both sides for the same BETR — but Vanguard's choice psychologically frames it as a cost rather than a migrated asset. The resulting BETR measures only the drag-alpha: the differential between sheltering $K$ inside the Roth wrapper versus exposing it to taxable-account drag. Our SRC framework restores what their quote alluded to, pricing the migrated \$35K explicitly at present value $K$ at the portfolio rate, additive to the drag-alpha rather than absorbed into a break-even rate adjustment.
