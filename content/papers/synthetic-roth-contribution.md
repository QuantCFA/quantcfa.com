...
**Passman, Wong, & Dickson (2025)** — *A "BETR" Approach to Roth Conversions*, Vanguard (July 2025) — extend the Kitces (2009) break-even framework by incorporating three factors that lower the indifference tax rate: the funding source for conversion tax, nondeductible basis, and future backdoor-Roth eligibility. Their derivation handles outside-account tax payment via a "forgone after-tax value" deduction — subtracting the future-grown conversion-tax dollar from the Roth side of the comparison while omitting it from the no-conversion taxable side, an accounting symmetry that yields the correct break-even rate without explicitly modeling wrapper migration. Vanguard's own sidebar nonetheless uses wrapper-shift language: *"To shift the full value of the IRA into a tax-advantaged account, pay conversion taxes with assets from a taxable account."* The math and the rhetoric are in tension — the SRC framework resolves this by making the wrapper migration explicit and quantifying the outside-payment value as a first-class PV / FV / IRR component, rather than embedding it in a symmetric-omission rate adjustment.
