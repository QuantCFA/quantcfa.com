...
The value of a Roth conversion is conventionally framed as the present-value impact of converting Traditional account dollars at one marginal tax rate today $t_C$ to avoid distributions at a different (typically higher) marginal tax rate later $t_D$. The "spread arbitrage" is the converted amount times $(t_D - t_C)$, modulated by time, growth, Social Security, and inflation.
