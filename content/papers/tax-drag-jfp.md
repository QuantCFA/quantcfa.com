...
Roth conversions generate tax-drag shelter (TDS) value by reducing the household's exposure to annual drag in taxable accounts, yet TDS is rarely priced as a distinct present-value line item in conversion analysis. This paper isolates two sources of that value, each tied to a specific planner decision: RMD-reduction alpha, unlocked by the Convert decision when conversions reduce future Required Minimum Distributions; and Synthetic Roth Contribution (SRC) alpha, unlocked by the Outside-settlement decision when conversion-tax dollars migrate into the Roth as a hidden Roth contribution. Both are shown to be special cases of a single valuation identity, $\alpha = P \cdot \mu$, where $P$ is the sheltered principal and $\mu$ is the per-dollar present value of the yearly distribution gap between parallel sheltered and taxable growth paths. The framework prices each alpha in closed form, attributes the corresponding share of Roth-conversion IRR to drag avoidance, and corrects the conventional Hidden Contribution figure's implicit zero-growth taxable counterfactual. RMD-reduction is the larger and more time-leveraged of the two — it scales with the full converted principal $C$ (versus SRC's $\tau_C \cdot C$) and grows substantially with longer owner life, while SRC's Phase-1 distribution mechanic is self-limiting. RMD-reduction applies whether conversion tax is paid Inside or Outside the wrapper; SRC alpha requires Outside funding. Across published treatments, tax drag appears as an embedded assumption, a breakeven condition, or a qualitative benefit, rather than as two separately priced alphas tied to specific planner decisions and to attributable shares of conversion IRR.
